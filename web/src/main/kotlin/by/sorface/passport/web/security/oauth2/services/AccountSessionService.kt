...
class DefaultAccountSessionService(private val sessionRepository: FindByIndexNameSessionRepository<out Session>) : AccountSessionService {
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(DefaultAccountSessionService::class.java)
    }

    private val logger = LoggerFactory.getLogger(javaClass)

    override fun findByUsername(username: String): List<Session> {
        logger.info("search active session by username [${username}]")
        return (sessionRepository.findByPrincipalName(username) ?: mapOf()).values.map { it }
    }
    ...
    override fun batchDeleteById(sessionIds: List<String>): Set<String> {
        logger.info("delete sessions by ids [${sessionIds}]")
