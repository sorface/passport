...
class ApplicationClientController(private val applicationClientFacade: ApplicationClientFacade) {

    @GetMapping
    fun findAllForCurrent(): List<ApplicationClient> = applicationClientFacade.findByCurrentUser()

    @GetMapping("/{clientId}")
    fun getById(@PathVariable("clientId") clientId: UUID): ApplicationClient? = applicationClientFacade.getByIdAndCurrentUser(clientId)

    @PatchMapping("/{clientId}/refresh")
    fun refreshSecret(@PathVariable("clientId") clientId: UUID): ApplicationClientRefreshSecret? = applicationClientFacade.refreshSecret(clientId)
    ...
    fun partialUpdate(@PathVariable("clientId") clientId: UUID, @RequestBody request: ApplicationClientPatchRequest): ApplicationClient? =
        applicationClientFacade.partialUpdate(clientId, request)
    ...
    @PostMapping
    fun registry(@RequestBody applicationRegistry: @Valid ApplicationRegistry): ApplicationClient? = applicationClientFacade.registry(applicationRegistry)
