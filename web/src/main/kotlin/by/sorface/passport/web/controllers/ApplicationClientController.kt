...
class ApplicationClientController {
    private val applicationClientFacade: ApplicationClientFacade? = null
    ...
    fun partialUpdate(
        @PathVariable("clientId") clientId: UUID?,
        @RequestBody request: ApplicationClientPatchRequest?
    ): ApplicationClient? {
    ...
    fun delete(
        @PathVariable clientId: @NotNull(message = I18Codes.I18ClientCodes.ID_MUST_BE_SET) @NotEmpty(message = I18Codes.I18ClientCodes.ID_CANNOT_BE_EMPTY) @Pattern(
            regexp = "^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$",
            message = I18Codes.I18ClientCodes.ID_IS_INVALID
        ) String?
    ): ResponseEntity<*> {
