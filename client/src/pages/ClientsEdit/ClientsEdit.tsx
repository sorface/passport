...
export const ClientsEdit: FunctionComponent = () => {
  const { apiMethodState, fetchData } = useApiMethod<ClientApp, string>(
    appsApiDeclaration.getById,
  );
  ...
  const { apiMethodState: editApiMethodState, fetchData: editFetch } =
  ...
  const { apiMethodState: deleteApiMethodState, fetchData: deleteFetch } =
  ...
  const fields: Field[] = [
    ...
    },
    {
      ...
      readOnly: true,
      defaultValue:
        data?.issueTime && new Date(data.issueTime).toLocaleString(),
    },
    {
      name: 'expiresAt',
