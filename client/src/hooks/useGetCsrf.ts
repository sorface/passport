...
const getCsrfConfigReducer = (
  state: CsrfConfigState,
  action: GetCsrfAction,
): CsrfConfigState => {
  switch (action.name) {
    case 'startLoad':
      return {
...
export const useCsrfApi = () => {
  const [csrfConfigState, dispatch] = useReducer(
    getCsrfConfigReducer,
    initialState,
  );
  ...
  const loadCsrfConfig = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        name: 'setError',
        payload:
          err instanceof Error ? err.message : 'Failed to get csrf config',
      });
    }
  }, []);
